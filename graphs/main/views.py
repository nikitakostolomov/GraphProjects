...
def segmentation(request):
    ...
    if request.method == 'POST':
        ...
        graph, k, result_img, tfm, tsm = start_algorithm(img_url, img_verify_url, object_pixels, background_pixels,
                                                         ...
                                                         lyambda=float(
                                                             request.POST.get('lyambda', '1') if request.POST.get(
                                                                 'lyambda', '') != '' else '1'),
                                                         sigma=float(
                                                             request.POST.get('sigma', '0.1') if request.POST.get(
                                                                 'sigma', '') != '' else '0.1'))
...
def interactive_segmentation(request):
    ...
    if request.method == 'POST':
        ...
        while stop:
            try:
                object_pixels += ' ' + Graph_and_pixels.objects.filter(id_graph=id_graph).values('object_pixels')[
                    0].get('object_pixels')
                background_pixels += ' ' + \
                                     Graph_and_pixels.objects.filter(id_graph=id_graph).values('background_pixels')[
                                         0].get('background_pixels')
                id_graph -= 1
            except:
                stop = False
